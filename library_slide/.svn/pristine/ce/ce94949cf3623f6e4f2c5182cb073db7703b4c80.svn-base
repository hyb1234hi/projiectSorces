...
public class CustomViewAbove extends ViewGroup {
	...
	/** added by sunny.dai to compat horizontal scroll */
	private boolean mScrollLeftEnable = true;
	private boolean mScrollRightEnable = true;

	public boolean isScrollLeftEnable() {
		return mScrollLeftEnable;
	}

	public boolean isScrollRightEnable() {
		return mScrollRightEnable;
	}

	public void setScrollLeftEnable(boolean mCanScrollLeft) {
		this.mScrollLeftEnable = mCanScrollLeft;
	}

	public void setScrollRightEnable(boolean mCanScrollLeft) {
		this.mScrollRightEnable = mCanScrollLeft;
	}
	...
	public void addUncontrolViews(View... views) {
		for (int i = 0; i < views.length; i++) {
			if (mUncontrolViews.contains(views[i]) && mUncontrolViews.get(mUncontrolViews.indexOf(views[i])) != null) {
				continue;
			}
	...
	public void removeAllUncontrolViews(){
		mUncontrolViews.clear();
	}
	...
	void setCurrentItemInternal(int item, boolean smoothScroll, boolean always, int velocity) {
		...
		// if left scroll disable
		if (!isScrollLeftEnable() && item == 2) {
			return;
		}
		// if left Right disable
		if (!isScrollRightEnable() && item == 0) {
			return;
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mUncontrolViews.size() > 0) {
			for (int i = 0; i < mUncontrolViews.size(); i++) {
				...
				winRect.set(l[0], l[1], l[0] + view.getWidth(), l[1] + view.getHeight());

				// Log.e("12345", winRect.left + "|" + px + "|" + winRect.right
				// + "|" + winRect.top + "|" + py + "|" + winRect.bottom);
				if (px < winRect.right && px > winRect.left && py < winRect.bottom && py > winRect.top) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mIsBeingDragged) {
				...
				/** MODIFY by sunny.dai */
				// if left disable , break;
				if (!isScrollLeftEnable() && scrollX > 0)
					break;
				// if right disable , break;
				if (!isScrollRightEnable() && scrollX < 0)
					break;
				/** MODIFY_END */
