...
final class LoadAndDisplayImageTask implements Runnable {
	...
	public LoadAndDisplayImageTask(Context context,ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler) {
		...
		Log.e("LoadAndDisplayImageTask  ", screenWidth + "   screenHeight  " + screenHeight);
	...
	private Bitmap tryLoadBitmap() {
		...
		if(bitmap != null && !bitmap.isRecycled()){
			Log.e("tryLoadBitmap", "tryLoadBitmap           " + bitmap.getWidth() +"       ---------     "+ bitmap.getHeight());
			bitmap = zoomBitmap(bitmap);
		}
		if(bitmap != null && !bitmap.isRecycled()){
			if(bitmap.getWidth() > screenWidth && screenWidth != 0){//如果图片的宽度大于屏幕宽度则缩放到合适的大小
				Matrix matrix = new Matrix();
				float inSampleSize;
				inSampleSize = (float)((float)screenWidth/(float)bitmap.getWidth());
	...
	/**
	 * 如果图片宽度或者高度大于4096  (android ImageView显示不支持)则递归缩放图片直到合适的大小
	 * @param bitmap
	 * @return
	 */
...
//			zoomBitmap(bitmap2);
			bitmap2 = resizeImage(bitmap,0.8f);
		}
	...
	public Bitmap resizeImage(Bitmap bitmap,float inSampleSize) {
		if(bitmap == null || bitmap.isRecycled()){
			return null;
		}
...
//        }
		if(resizedBitmap != null && !resizedBitmap.isRecycled())
			resizedBitmap = zoomBitmap(resizedBitmap);
