...
public class NetImageView extends ImageView {
	...
	public void recycle(boolean showDefault) {
		ImageLoader.getInstance(null).cancelDisplayTask(this);
		if (showDefault) {
	...
	public void reload(final boolean loadBig) {
		if (!isLoad && !TextUtils.isEmpty(url)) {
			isLoad = true;
			setImageResource(defaultImageRes);
			ImageLoader.getInstance(null).displayImage(url, this, options,
					new ImageLoadingListener() {
						...
						public void onLoadingComplete(String imageUri,
							...
							if (loadBig && !TextUtils.isEmpty(urlBig)) {
								ImageLoader.getInstance(null).displayImage(urlBig,
