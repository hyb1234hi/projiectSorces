...
/**
 * 在异常发生时会触发,会把异常信息打印并显示到终端设备上.多用于debug测试阶段</br>
 * 注册  {@link ErrorHandler} 方式: </br>
 * Thread.setDefaultUncaughtExceptionHandler(new ErrorHandler(currentActivity));
 * </br></br>
 * 
 * 或者调用 {@link ErrorHandler#registerNewErrorHandler(Activity)}
 * 方法注册异常监听.</br></br>
 * 
 * 增加电子邮件的支持, 调用
 * {@link ErrorHandler#enableEmailReports(String, String)} </br></br>
 * 
 *  ErrorHandler 必须在 AndroidManifest.xml 里注册:
 * 
...
public class ErrorHandler extends Activity implements UncaughtExceptionHandler {
	...
	/**
	 * 必须有相同的“"x/y" 字串在AndroidManifest</br>
	 * </br>
	 * 参考 {@link ErrorHandler} 下的注释配置 AndroidManifest 清单
	 */
...
//	private static String myDeveloperMailAdress = "perry.li@itotem.com.cn";
	...
	private  void loadErrorLayout(Activity a, String myErrorText) {
		...
		LayoutParams paramsEdit = new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT);
		paramsEdit.weight = 1;
		 ...
		 LayoutParams paramsButton = new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT);
...
//			myTextView.setEnabled(true);
			...
			handler.post(new Runnable(){
				...
				public void run() {
					...
					ErrorLogSave.cacheErrorLogToSDFile(ErrorHandler.this,myTextView.getText().toString());
				}
			});
		}
	}

	private  void enableMailButton(final Activity a,
	...
	public void uncaughtException(final Thread thread, final Throwable ex) {
		...
		if (myCurrentActivity != null) {
			...
			handler.post(new Runnable(){
				...
				public void run() {
					// TODO Auto-generated method stub
					String debugText = CollectDataManager.getDebugInfosToErrorMessage(ErrorHandler.this);
					ErrorLogSave.cacheErrorLogToSDFile(ErrorHandler.this,throwableToString(ex)+debugText);
				}
			});
